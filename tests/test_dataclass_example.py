...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'examples', 'dataclass')))

from app import app, pets as initial_pets, PetIn, PetOut
...
class TestDataclassExample:
    ...
    def test_create_pet_with_default_category(self, client, reset_pets):
        response = client.post(
            '/pets',
            json={'name': 'Buddy'}
        )
        data = json.loads(response.data)
        assert response.status_code == 201
        assert data['category'] == 'dog'  # Default value
    ...
    def test_create_pet_invalid_data(self, client):
        ...
        response = client.post(
            '/pets',
            json={'name': '', 'category': 'dog'}
        )
        assert response.status_code == 422

    def test_update_pet_success(self, client, reset_pets):
        # Note: Unlike the basic example, partial updates aren't supported here
    ...
    def test_pet_dataclass_instantiation(self):
        pet_in = PetIn(name='Max', category='dog')
        assert pet_in.name == 'Max'
        assert pet_in.category == 'dog'

        pet_out = PetOut(id=1, name='Max', category='dog')
        assert pet_out.id == 1
        assert pet_out.name == 'Max'
        assert pet_out.category == 'dog'
